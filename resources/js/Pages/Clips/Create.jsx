...
export default function Create({ episode }) {
    ...
    const stepItems = useMemo(() => ([
        { key: 'create', label: 'Create clip entry' },
        ...
        { key: 'update', label: 'Update clip with Bunny data' },
    ]), []);
    ...
    const waitForBunnyProcessing = async (videoId, libraryId) => {
        ...
        for (let i = 0; i < maxAttempts; i += 1) {
            const response = await axios.post(route('episodes.clips.bunny.status', episode.id), {
                video_id: videoId,
        ...
        try {
            for (let i = startIndex; i < stepOrder.length; i += 1) {
                ...
                if (currentStep === 'create') {
                    setStatusMessage('Creating clip entry...');
                    const createResponse = await axios.post(route('episodes.clips.store', episode.id), {
                        title: data.title,
                        slug: data.slug,
                    ...
                    if (!videoId) throw new Error('Clip was created but ID was not returned.');
                ...
                if (currentStep === 'upload') {
                    ...
                    if (!context.session || context.session.expires < Math.floor(Date.now() / 1000) + 60) {
                        const initResponse = await axios.post(route('episodes.clips.bunny.init', episode.id), { title: data.title });
                ...
                if (currentStep === 'thumbnail') {
                    ...
                    await axios.post(route('episodes.clips.bunny.thumbnail', episode.id), formData, {
                ...
                if (currentStep === 'update') {
                    setStatusMessage('Updating clip...');
                    await axios.patch(route('episodes.clips.update', [episode.id, context.videoId]), {
                        title: data.title,
                        slug: data.slug,
                        short_description: data.short_description,
                        long_description: data.long_description,
            ...
            setStatusMessage('Clip created successfully. Redirecting...');
            setWorkflowDone(true);
            await wait(900);
            window.location.href = route('episodes.clips.index', episode.id);
        } catch (err) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-slate-200">Add clip</h2>}>
            <Head title="Add clip" />
            ...
            <div className="mx-auto w-full max-w-6xl">
                <form onSubmit={handleSubmit} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-gray-800">
                    <div className="border-b border-gray-200 bg-white px-6 py-5 dark:border-slate-700 dark:bg-gray-800">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Clip Upload</h3>
                    ...
                    <div className="space-y-6 p-6 md:p-8">
                        ...
                        <div className="flex justify-end">
                            <PrimaryButton type="submit">Save clip</PrimaryButton>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <Modal show={showProcessingModal} onClose={() => {}} closeable={false} maxWidth="lg">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Processing clip</h3>
