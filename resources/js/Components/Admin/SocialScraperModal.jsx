...
/* ─────────────────────────────────────────────────────────────────────────
   Hosts that reliably need the real-browser Agent mode
───────────────────────────────────────────────────────────────────────── */
...
function hostNeedsAgent(rawUrl) {
    try {
        ...
        return AGENT_HOSTS.has(hostname);
    } catch {
        return false;
    }
}
...
/* ─────────────────────────────────────────────────────────────────────────
   Main modal
───────────────────────────────────────────────────────────────────────── */
export default function SocialScraperModal({ show, onClose }) {
    const [url, setUrl] = useState('');
    const [mode, setMode] = useState('auto'); // 'auto' | 'lite' | 'agent'
    const [groups, setGroups] = useState([]);
    const [running, setRunning] = useState(false);
    const [error, setError] = useState(null);
    const abortRef = useRef(null);   // for Lite (SSE) AbortController
    ...
    const updateGroup = useCallback((id, updater) => {
        setGroups((prev) =>
            prev.map((g) => (g.id === id ? { ...g, ...updater(g) } : g)),
        );
    }, []);
    ...
    // Decide effective mode: if 'auto', pick agent for known social hosts
    const effectiveMode = (rawUrl) => {
        if (mode === 'auto') return hostNeedsAgent(rawUrl) ? 'agent' : 'lite';
        return mode;
    };
    ...
    return (
        <Modal show={show} onClose={onClose} maxWidth="3xl">
            <div className="flex max-h-[90vh] flex-col">
                {/* ── Header ── */}
                <header className="flex items-center justify-between border-b border-gray-200 bg-gradient-to-br from-[#fff8e0] via-white to-[#fff4cc] px-5 py-4 dark:border-slate-700 dark:from-slate-800 dark:via-slate-800 dark:to-slate-800/80">
                    <div className="flex items-center gap-2">
                        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#b59100]/10 text-[#b59100]">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-slate-400">
                                Paste a public social URL — Gemini will fetch
                                recent posts.
                            </p>
                        </div>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-lg p-1.5 text-gray-400 transition hover:bg-white/60 hover:text-gray-700 dark:hover:bg-slate-700 dark:hover:text-slate-200"
                        aria-label="Close"
                    >
                        <X className="h-4 w-4" />
                    </button>
                ...
                <div className="flex flex-col gap-4 overflow-y-auto px-5 py-4">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                        ...
                        <button
                            ...
                            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md bg-[#b59100] px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#9a7c00] disabled:cursor-not-allowed disabled:bg-gray-300 dark:disabled:bg-slate-600"
                        >
                            {running ? (
                    ...
                    {running && (
                        <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
                            ...
                            Don't refresh the page — Gemini is working on your
                            scrape.
                        </div>
                    )}

                    {/* ── Live view + thinking (side by side when agent is active) ── */}
                    {activeGroup &&
                ...
                <footer className="border-t border-gray-200 bg-gray-50 px-5 py-3 text-xs text-gray-500 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-400">
                    Saving is not wired up yet — cards are kept in memory until
...
/* ─────────────────────────────────────────────────────────────────────────
   Live View — streams JPEG frames from the sidecar WebSocket
───────────────────────────────────────────────────────────────────────── */
function LiveView({ liveWsUrl, status, onCancel }) {
    ...
    return (
        ...
        >
            ...
            {/* frame — compact by default; fullscreen via expand button */}
            <motion.div
                className={`relative flex min-h-0 items-center justify-center bg-gray-950 ${
                    fullscreen ? 'h-[calc(100%-2.5rem)]' : 'h-auto'
                }`}
            >
                {frame ? (
                    <img
    ...
    return (
        ...
        >
            <div className="flex min-w-0 items-center justify-between gap-2 border-b border-gray-100 px-3 py-2 text-xs font-medium text-gray-500 dark:border-slate-700 dark:text-slate-400">
                ...
                <span className="min-w-0 truncate text-[11px] text-gray-400 dark:text-slate-500">
                    {status}
                </span>
            </div>
            <div
            ...
            >
                {joined || (
                    <span className="italic text-gray-400 dark:text-slate-500">
                        Waiting for first thought from Gemini…
                    </span>
                )}
            </div>
        </motion.div>
...
function ScrapeCard({ group, onRemove, onRemovePost, onSavedLoaded, onSave, onTogglePostActive }) {
    const isRunning = ['starting', 'connecting', 'streaming', 'running'].includes(
        group.status,
    );
    ...
    return (
        ...
        >
            <header className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-slate-700">
                <div className="flex min-w-0 items-center gap-2">
                    <span className="inline-flex h-6 min-w-6 items-center justify-center rounded-full bg-[#b59100]/10 px-2 text-xs font-semibold text-[#b59100]">
                        Scrape {group.index}
                    </span>
                    ...
                    <a
                        ...
                        className="truncate text-sm font-medium text-gray-700 hover:text-[#b59100] dark:text-slate-200"
                        title={group.url}
                    >
                        {group.host}
                    </a>
                ...
                <div className="flex shrink-0 items-center gap-2">
                    {(isDone || isAborted) && group.posts.length > 0 && (
                        <button
                            ...
                            className="inline-flex items-center gap-1 rounded-md bg-[#b59100] px-2.5 py-1 text-xs font-semibold text-white hover:bg-[#9a7c00] disabled:opacity-60"
                        >
                            {group.saving
                    ...
                    <button
                        type="button"
                        onClick={onRemove}
                        className="inline-flex items-center gap-1 rounded-md border border-gray-200 px-2 py-1 text-xs text-gray-500 hover:border-red-200 hover:bg-red-50 hover:text-red-600 dark:border-slate-600 dark:text-slate-400 dark:hover:border-red-900/50 dark:hover:bg-red-900/20 dark:hover:text-red-300"
                    >
            ...
            <div className="px-4 py-3">
                {group.saveMessage && (
                    <p className="mb-2 text-xs font-medium text-emerald-700 dark:text-emerald-300">{group.saveMessage}</p>
                )}
                {group.saveError && (
                    <p className="mb-2 text-xs text-red-600 dark:text-red-300">{group.saveError}</p>
                )}
                {group.savedMeta && (
                    <div className="mb-3 rounded-md border border-sky-200 bg-sky-50/80 px-3 py-2 text-xs text-sky-900 dark:border-sky-900/50 dark:bg-sky-900/20 dark:text-sky-100">
                        <span className="font-semibold">Previously saved</span>
                        {' · '}
                        {platformLabel(group.savedMeta.platform)}
                        {' · '}
                        {group.savedMeta.posts_count} posts ({group.savedMeta.active_count} active)
                    </div>
                )}
                {isError && (
                    <div className="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300">
                ...
                {isAborted && (
                    <p className={`text-sm italic text-gray-500 dark:text-slate-400 ${group.posts.length > 0 ? 'mb-3' : ''}`}>
                        Cancelled — {group.posts.length > 0 ? `${group.posts.length} post(s) collected before cancellation.` : 'No posts collected.'}
                    </p>
                )}
                ...
                {!isError && (
                    <>
                        {group.parsed?.notes && (
                            <p className="mb-3 rounded-md border border-amber-100 bg-amber-50/60 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
                        ...
                        {group.posts.length > 0 ? (
                            ...
                            <p className="text-sm italic text-gray-500 dark:text-slate-400">
                                No posts could be extracted from this URL.
                            </p>
                        ) : isRunning ? (
                            <p className="text-sm italic text-gray-500 dark:text-slate-400">
                                Working…
...
function StatusBadge({ status, count }) {
    ...
    return (
        <span
            className={`ml-1 inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${v.cls}`}
        >
        ...
        <article className={`relative flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-slate-700 dark:bg-slate-900/40 ${!isActive ? 'opacity-60' : ''}`}>
            ...
            <div className="relative flex aspect-video w-full items-center justify-center overflow-hidden bg-gray-100 dark:bg-slate-800">
                {media ? (
                    <img
                        src={media}
                        alt={post.description?.slice(0, 80) ?? 'Post thumbnail'}
                        className="h-auto w-full object-contain"
                        onError={(e) => {
            ...
            <div className="flex flex-1 flex-col gap-2 p-3">
                ...
                {post.description && (
                    <p className="line-clamp-4 whitespace-pre-wrap text-xs leading-relaxed text-gray-700 dark:text-slate-300">
                        {post.description}
                    </p>
                )}
...
function EmptyState() {
    return (
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-5 py-10 text-center dark:border-slate-700 dark:bg-slate-900/40">
            <Sparkles className="mx-auto h-6 w-6 text-[#b59100]" />
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-slate-400">
                Auto mode picks the real-browser Agent for Instagram, LinkedIn
                etc., and the fast Lite path for everything else.
            </p>
        </div>
    );
}
...
/* ─────────────────────────────────────────────────────────────────────────
   Lite scrape (existing Gemini url_context SSE flow)
───────────────────────────────────────────────────────────────────────── */
    ...
    try {
        ...
        for (;;) {
            ...
            while ((sep = buffer.indexOf('\n\n')) !== -1) {
                ...
                const parsedEvt = parseSseBlock(block);
                if (!parsedEvt) continue;
                updateGroup(id, (g) => applyLiteEvent(g, parsedEvt.event, parsedEvt.data));
            }
        }

        updateGroup(id, (g) => {
            if (g.parsed || !g.textBuffer) return { finishedAt: Date.now() };
            const parsedJson = extractJson(g.textBuffer);
            return {
                parsed: parsedJson,
                posts: parsedJson?.posts ?? [],
                status: g.status === 'error' ? g.status : 'done',
                statusMessage: parsedJson ? 'Scrape complete.' : 'Finished, no structured posts found.',
                finishedAt: Date.now(),
            };
        });
    } catch (err) {
...
/* ─────────────────────────────────────────────────────────────────────────
   Agent scrape (Chromium + Gemini vision, WebSocket)
───────────────────────────────────────────────────────────────────────── */
async function runAgentScrape(id, trimmed, updateGroup, setRunning) {
    try {
            ...
            eventsWs.onclose = (e) => {
                updateGroup(id, (g) => {
                    ...
                    if (g.posts.length > 0) {
                        return {
                            status: 'done',
                            statusMessage: `Connection closed — ${g.posts.length} post(s) collected.`,
                            finishedAt: Date.now(),
                        };
                    }
                    if (e.code !== 1000 && e.code !== 1001) {
                        return {
                            status: 'error',
                            statusMessage: 'Connection closed unexpectedly.',
                            errorMessage: `WS closed (code ${e.code})`,
                            finishedAt: Date.now(),
                        };
                    }
        ...
        updateGroup(id, () => ({
            status: 'error',
            statusMessage: 'Agent failed to start.',
            errorMessage: err.message,
            finishedAt: Date.now(),
        }));
        setRunning(false);
...
function applyLiteEvent(group, event, data) {
    switch (event) {
        case 'open':
            return { status: 'connecting', statusMessage: 'Connected. Waiting for Gemini…' };
        case 'status':
            return {
                status: data.stage === 'streaming' ? 'streaming' : group.status,
                statusMessage: data.message ?? group.statusMessage,
            };
        case 'thought':
            return { status: 'streaming', thoughts: [...group.thoughts, data.text], statusMessage: 'Thinking…' };
        case 'text':
            return { status: 'streaming', textBuffer: group.textBuffer + data.text, statusMessage: 'Writing result…' };
        case 'urlMeta':
            return { urlsFetched: Array.isArray(data.urls) ? data.urls : group.urlsFetched };
        case 'finish': {
        ...
        case 'done': {
            if (group.status === 'done' || group.status === 'error') return {};
            const parsed = extractJson(group.textBuffer);
            const posts = (parsed?.posts ?? []).map(normalizePostFromScrape);
            return {
                status: 'done',
...
function applyAgentEvent(group, type, data) {
    ...
    switch (type) {
        case 'connected':
            return { statusMessage: tag + 'Browser connected.' };
        case 'status':
            return { statusMessage: tag + (data.message ?? group.statusMessage) };
        case 'thought':
            return { thoughts: [...group.thoughts, (tag ? tag : '') + data.text], statusMessage: tag + 'Thinking…' };
        case 'text':
            return { textBuffer: group.textBuffer + data.text };
        case 'action':
            return { statusMessage: tag + `${data.name}(${JSON.stringify(data.args ?? {}).slice(0, 60)})` };
        case 'post': {
        ...
        case 'error': {
            ...
            // Treat a cancel-induced error as a cancellation so collected posts aren't hidden
            if (msg === 'cancelled') {
                return {
                    status: 'cancelled',
                    statusMessage: group.posts.length > 0
