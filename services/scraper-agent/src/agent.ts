...
import { AgentEvent, Credentials, Post, ScrapeResult } from './types.js';
...
export function planWorkerSlices(totalPosts: number = MAX_POSTS): WorkerSlice[] {
  return planTabSlices(
    totalPosts,
    PARALLEL_THRESHOLD,
    POSTS_PER_WORKER,
    parseInt(process.env.MAX_CONCURRENT_SESSIONS ?? '8', 10) || 8,
  );
}
    ...
    functionDeclarations: [
      {
        name: 'screenshot',
        description:
          'Capture a screenshot of the current browser viewport and return it so you can see the page state.',
      ...
      },
      {
        name: 'click',
        description:
          'Click at an (x, y) coordinate in the viewport. Prefer clickText when you know the element label.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            x: { type: Type.NUMBER, description: 'X pixel coordinate.' },
            y: { type: Type.NUMBER, description: 'Y pixel coordinate.' },
          },
          required: ['x', 'y'],
        },
      },
      {
        name: 'clickText',
        description:
          'Click the first visible element whose text or accessible label matches the given string.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            text: {
      ...
      },
      {
        name: 'type',
        description:
          'Type text into the currently focused element (e.g. after clicking an input).',
        parameters: {
          type: Type.OBJECT,
          properties: {
            text: {
      ...
      },
      {
        name: 'fillField',
        description:
          'Reliably fill a form input or textarea by CSS selector. ' +
      ...
      },
      {
        name: 'clickSelector',
        description:
          'Click an element on the page identified by a CSS selector — more reliable than coordinate-based click for buttons and links. ' +
      ...
      },
      {
        name: 'wait',
        description:
          'Wait for a given number of milliseconds (max 5000ms) to allow the page to load.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            ms: {
      ...
      },
      {
        name: 'submitPost',
        description:
          'Stream a single completed post to the UI immediately after collecting it. ' +
      ...
      },
      {
        name: 'extractPosts',
        description:
          'Finalise the scrape session. Call this ONLY after you have submitted all posts via submitPost. ' +
        ...
        parameters: {
          ...
          properties: {
            platform: {
              type: Type.STRING,
              description:
                'Platform name: instagram, linkedin, twitter, youtube, facebook, tiktok, or other.',
            },
            posts: {
              type: Type.ARRAY,
      ...
      },
      {
        ...
        description:
          'Extract all post data directly from the Instagram profile grid DOM in one call. ' +
      ...
      },
      {
        name: 'getBoundingBox',
        description:
          'Scroll a DOM element to the centre of the viewport, then return its exact pixel bounding box. ' +
      ...
      },
      {
        name: 'verifyCapture',
        description:
          'Load a previously captured image file and return it so you can visually inspect it. ' +
      ...
      },
      {
        name: 'captureArea',
        description:
          'Crop and save a specific rectangular region of the current viewport as an image file. ' +
      ...
      },
      {
        name: 'captureElement',
        description:
          'Capture a screenshot of a specific DOM element by CSS selector and save it as an image file. ' +
      ...
      },
      {
        name: 'getPageSource',
        description:
          'Return a compact summary of the current page\'s interactive elements: all buttons (with text/type/id/class), ' +
      ...
      },
      {
        ...
        description:
          'Read a property or attribute from a DOM element on the current page using a CSS selector. ' +
      ...
      },
      {
        ...
        description:
          'Like getDomAttribute but returns values from ALL matching elements as an array. ' +
      ...
      },
      {
        name: 'finish',
        description:
          'Signal that the scrape is complete or impossible to continue.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            reason: {
...
function mergeTabIndexIntoTools(raw: Tool[]): Tool[] {
  const tabIdx = {
    tabIndex: {
      type: Type.NUMBER,
      description:
        'Browser tab index (0-based). When multiple tabs are open you MUST set this on every tool to the tab you are acting on. For a single tab, use 0 or omit.',
    },
  };
  return raw.map((tool) => ({
    ...tool,
    functionDeclarations: (tool.functionDeclarations ?? []).map((fd) => {
      const p = fd.parameters as {
        type: Type;
...
const TOOLS: Tool[] = mergeTabIndexIntoTools(TOOLS_RAW);

/* ---------- executor helpers ---------- */

type ToolArgs = Record<string, unknown>;

function resolveToolPage(pages: Page[], args: ToolArgs): Page {
...
async function execTool(
  ...
  switch (name) {
    ...
    case 'extractInstagramGrid': {
      ...
      const posts = await page.evaluate<any[]>(() => {
        ...
        for (const a of anchors) {
          ...
          if (!href) continue;
          const mediaSrc: string = img.src ?? img.getAttribute('src') ?? '';
          const caption: string = img.getAttribute('alt') ?? '';
          const isPinned: boolean = !!a.querySelector('svg[aria-label="Pinned post icon"]');
          const isReel: boolean = href.includes('/reel/');
    ...
    case 'getBoundingBox': {
      ...
      try {
        ...
        if (count === 0) return { error: `No element found for selector: ${sel}` };
        ...
        if (!box) return { error: `Element found but not visible/measurable after scrolling: ${sel}` };
    ...
    case 'getPageSource': {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const summary = await page.evaluate<any>(() => {
        // Runs in browser context — all DOM APIs available at runtime
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const d = (globalThis as any).document;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const toSel = (el: any): string => {
          if (el.id) return `#${el.id}`;
          const tag = String(el.tagName ?? 'el').toLowerCase();
          const name = el.getAttribute?.('name');
          const type = el.getAttribute?.('type');
          const cls = el.classList ? Array.from(el.classList as string[]).slice(0, 2).join('.') : '';
    ...
    case 'getDomAttribute': {
      ...
      const value = await page.evaluate<string | null, { s: string; a: string }>(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ({ s, a }) => {
    ...
    case 'getDomAttributeAll': {
      const sel  = String(args.selector);
      const attr = String(args.attribute);
      const values = await page.evaluate<(string | null)[], { s: string; a: string }>(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ({ s, a }) => {
          const els: any[] = Array.from((globalThis as any).document.querySelectorAll(s));
      ...
      const unique = [...new Set(values.filter((v): v is string => typeof v === 'string' && !!v))];
...
export function resolveCredentials(
  ...
  if (hostname.includes('instagram.com')) {
    ...
    if (u && p) return { id: 'env-instagram', platform: 'instagram', username: u, password: p };
  }

  if (hostname.includes('linkedin.com')) {
    const u = process.env.LINKEDIN_USERNAME ?? '';
    const p = process.env.LINKEDIN_PASSWORD ?? '';
    if (u && p) return { id: 'env-linkedin', platform: 'linkedin', username: u, password: p };
  }
...
export async function runAgent(
  ...
  if (pages.length === 0) throw new Error('No browser pages');
  ...
  const tabLabel = (idx: number): string =>
    multi ? `[T${idx + 1}] ` : '';
  ...
  const history: ReturnType<typeof createUserContent>[] = [];

  let activeMs = 0;
  let result: ScrapeResult | null = null;
  const streamedPosts: Post[] = [];
  const collectedByTab = new Array(slices.length).fill(0);

  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    if (signal.aborted) throw new Error('cancelled');
    if (activeMs > MAX_ACTIVE_MS) throw new Error('timeout');

    const userPieces: Array<ReturnType<typeof createPartFromText> | ReturnType<typeof createPartFromBase64>> = [];
    const tabScreenshots = await Promise.all(
      pages.map((p, ti) =>
        p.screenshot({ type: 'jpeg', quality: multi ? 52 : 60 }).then((buf) => ({ ti, buf })),
      ),
    );
    for (const { ti, buf } of tabScreenshots) {
      userPieces.push(createPartFromBase64(buf.toString('base64'), 'image/jpeg'));
      const sl = slices[ti]!;
      userPieces.push(
        createPartFromText(
          `tabIndex ${ti}${multi ? ` — progress ${collectedByTab[ti]}/${sl.postLimit} posts (grid indices ${sl.postOffset}–${sl.postOffset + sl.postLimit - 1})` : ''}.`,
        ),
      );
    }
    userPieces.push(
    ...
    history.push(createUserContent(userPieces as Parameters<typeof createUserContent>[0]));
    ...
    const modelParts: ReturnType<typeof createPartFromText>[] = [];
    let calledTerminalTool = false;

    type FnCallPart = {
      part: ReturnType<typeof createPartFromText>;
      toolName: string;
      toolArgs: ToolArgs;
    };
    const fnCalls: FnCallPart[] = [];
    ...
    for (const part of candidate.content?.parts ?? []) {
      ...
      if (typedPart.functionCall) {
        fnCalls.push({
          part: part as ReturnType<typeof createPartFromText>,
          toolName: String(typedPart.functionCall.name ?? ''),
          toolArgs: (typedPart.functionCall.args ?? {}) as ToolArgs,
        });
        continue;
      }
      ...
      if (typedPart.text && !typedPart.thought) {
        ...
        modelParts.push(part as ReturnType<typeof createPartFromText>);
      }
    }

    const appendToolResponse = (
      toolName: string,
      toolArgs: ToolArgs,
      toolResult: unknown,
    ): void => {
      if (
        toolName === 'verifyCapture' &&
        toolResult &&
        typeof (toolResult as Record<string, unknown>).imageBase64 === 'string'
      ) {
        const res = toolResult as { imageBase64: string; mimeType: string };
        modelParts.push(
          createPartFromFunctionResponse(
            toolName, toolName,
            { status: 'captured', media_url: String(toolArgs.media_url ?? '') },
          ) as unknown as ReturnType<typeof createPartFromText>,
        );
        modelParts.push(
          createPartFromBase64(res.imageBase64, res.mimeType) as unknown as ReturnType<typeof createPartFromText>,
        );
        modelParts.push(
          createPartFromText('Above is the captured thumbnail image. Verify it shows post content (not a profile pic, nav bar, or blank area).') as unknown as ReturnType<typeof createPartFromText>,
        );
      } else {
        modelParts.push(
          createPartFromFunctionResponse(
    ...
    while (callIdx < fnCalls.length && !calledTerminalTool) {
      ...
      if (toolName === 'submitPost') {
        ...
        const post = normalizePost({ ...(toolArgs as Partial<Post>), type: 'image' });
        streamedPosts.push(post);
        const tabIdx =
          typeof toolArgs.tabIndex === 'number' && !Number.isNaN(toolArgs.tabIndex)
      ...
      if (toolName === 'extractPosts') {
        ...
        modelParts.push(part);
        const batchPosts = ((toolArgs.posts as Post[]) ?? []).map(normalizePost);
      ...
      if (toolName === 'finish') {
        ...
        const notes =
          reason === 'login_required' && resolvedCreds
      ...
      if (batch.length > 1) {
        ...
        const results = await Promise.all(
          batch.map((b) => execTool(pages, b.toolName, b.toolArgs)),
        );
        activeMs += Date.now() - toolStart;
    ...
    if (modelParts.length > 0) {
      history.push({
        role: 'model',
        parts: modelParts,
      } as unknown as ReturnType<typeof createUserContent>);
    }

    if (calledTerminalTool) break;
  }
...
function redactCredentials(
  ...
  if (!credentials) return args;
  const replace = (v: string): string =>
    v
      .replace(
        new RegExp(escapeRe(credentials.username), 'g'),
        '[username]',
      )
      .replace(
        new RegExp(escapeRe(credentials.password), 'g'),
        '[password]',
      );
  const safe: ToolArgs = {};
